...
- 🔍 **单点检测**:顶部捕获框聚焦后按下想设的热键(如 `Ctrl+Alt+J`),即时探测这一个,无需全量扫描
- 🩺 **双击行 → 详情诊断面板**:VK码 / 作用域 / 来源证据链 / 排障建议 / 一键复制诊断信息
- 🔗 **来源证据链 + 置信度**:占用项标注 `✓进程在运行` / `✓命中默认热键` / `✗API无法直接确认`,附星级置信度,不再"瞎猜"
- 🎛️ **可配置扫描范围**:字母 / 数字 / 功能键 / 导航键 / 符号键 / 小键盘 / 多媒体·浏览器键 / Win 组合
